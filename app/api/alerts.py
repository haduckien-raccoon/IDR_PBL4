...
# @router.post("/analyze")
# async def analyze_alert(request: Request, session: Session = Depends(get_session)):
#     try:
#         payload = await request.json()
#         payload = payload or {}

#         qp = request.query_params
#         force_email = as_bool(qp.get("force_email"), False) or as_bool(payload.get("force_email"), False)

#         analyzer = Analyzer()
#         result = analyzer.analyze(payload)

#         if not getattr(result, "should_alert", False) and not force_email:
#             return JSONResponse(status_code=200, content={
#                 "status":"success",
#                 "message":"Analysis complete - no alert needed",
#                 "email_attempted": False
#             })

#         attack_name = getattr(result, "alert_type", None) or payload.get("attack_name") or "Unknown"
...
#         src_ip = payload.get("src_ip") or "0.0.0.0"
#         dst_ip = payload.get("dst_ip") or "0.0.0.0"
#         desc = getattr(result, "description", payload.get("description"))
...
#         level = map_severity_to_alert_level(e.severity)
#         alert_message = getattr(result, "description", None) or f"{attack_name} detected"
#         a = Alert(
#             event_id = e.event_id,
#             alert_message = alert_message,
#             alert_level = level,
...
#             "destination_ip": str(e.destination_ip) if e.destination_ip else None,
#             # [THÊM MỚI] Đảm bảo payload của analyzer cũng được đính kèm
#             "captured_path": payload.get('captured_file'),
...
#         email_attempted = False
#         # Gửi email trong thread để không block event loop
#         if force_email or True:
#             try:
#                 await to_thread.run_sync(_send_email_sync, email_payload)
#                 a.is_sent = True
#                 email_attempted = True
#             except Exception as mail_err:
#                 logger.error(f"Email error for alert #{a.alert_id}: {mail_err}")

#         # Phát realtime tới WebSocket clients (không chờ – fire-and-forget)
#         try:
#             asyncio.create_task(emit_alert_realtime(a, e))
#         except RuntimeError:
#             await emit_alert_realtime(a, e)
...
#         return JSONResponse(status_code=500, content={"status":"error","message":"Failed to analyze alert data"})
...
async def analyze_alert(request: Request, session: Session = Depends(get_session)):
    try:
        ...
        session.add(e)
        session.flush()
        logger.info(f"Created Event #{e.event_id} with severity={severity}")
        ...
        email_attempted = False
        if force_email or True:
            logger.info(f"Preparing to send email for Alert #{a.alert_id}")
            ...
            try:
                ...
                a.is_sent = True
                email_attempted = True
                logger.info(f"Email successfully sent for Alert #{a.alert_id}")
            except Exception as mail_err:
                logger.exception(f"Email error for Alert #{a.alert_id}: {mail_err}")
