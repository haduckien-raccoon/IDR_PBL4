...
class IDS:
    ...
    def log_traffic(self, meta: Dict[str, Any], payload: bytes):
        ...
        try:
            ...
            with self._logged_payloads_lock:
    ...
    def match_payload(self, payload: bytes, meta: Dict[str, Any]):
        ...
        behavior = self.behavior_inspector.process(meta, http_uri, status_code)
        if behavior is not None:
            # If behavior is a list, use the first element
            if isinstance(behavior, list) and behavior:
                b = behavior[0]
                self.log_alert(meta, p, b.get("rid", ""), b.get("message", ""), b.get("variant", ""), b.get("action", ""), b.get("severity", ""))
            elif isinstance(behavior, dict):
                self.log_alert(meta, p, behavior.get("rid", ""), behavior.get("message", ""), behavior.get("variant", ""), behavior.get("action", ""), behavior.get("severity", ""))
            else:
                # fallback: log as unknown
                self.log_alert(meta, p, "", str(behavior), "", "", "")
            return
        # --- prepare hits list ---
